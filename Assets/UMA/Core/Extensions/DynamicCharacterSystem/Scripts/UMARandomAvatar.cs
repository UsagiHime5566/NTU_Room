	...
	public class UMARandomAvatar : MonoBehaviour
	{
		...
		public void GenerateRandomCharacter(Vector3 Pos, Quaternion Rot, string Name)
		{
			if (prefab)
			{
				...
				anim.SetInteger("Rand", Random.Range(0, 6));
				StartCoroutine(DelaySize(go.transform));
		...
		IEnumerator DelaySize(Transform ts){
			yield return new WaitForSeconds(delaySetting);
			ts.localScale  = Vector3.one * scaleSize;

			Animator anim = ts.GetComponent<Animator>();
			while(true){
